...
//Super of other classes
    ...
    it.describe("#format", function (it) {
        it.should("format EEEE, MMMM dd, yyyy to Friday, August 11, 2006 ", function () {
            ...
            assert.equal(comb.date.format(date, "EEEE, MMMM dd, yyyy"), "Friday, August 11, 2006");
        });

        it.should("format EEEE, MMMM dd, yyyy G to Friday, August 11, 2006 AD ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, "EEEE, MMMM dd, yyyy G"), "Friday, August 11, 2006 AD");
        });

        it.should("format EEEE, MMMM dd, yyyy GGGG to Friday, August 11, 2006 Anno Domini ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, "EEEE, MMMM dd, yyyy GGGG"), "Friday, August 11, 2006 Anno Domini");
        });

        it.should("format M/dd/yy  to 8/11/06 ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, "M/dd/yy"), "8/11/06");
        });

        it.should("format E  to 6 ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, "E"), "6");
        });

        it.should("format h:m a  to 12:55 AM ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, "h:m a"), "12:55 AM");
        });

        it.should("format H:m:s  to 12:55:12", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, 'h:m:s'), "12:55:12");
        });

        it.should("format h:m:s.SS  to 12:55:12.35 ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, 'h:m:s.SS'), "12:55:12.35");
        });

        it.should("format k:m:s.SS to 24:55:12.35 ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, 'k:m:s.SS'), "24:55:12.35");
        });

        it.should("format K:m:s.SS to 0:55:12.35 ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, 'K:m:s.SS'), "0:55:12.35");
        });

        it.should("format H:m:s.SS to 0:55:12.35 ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, 'H:m:s.SS'), "0:55:12.35");
        });


        it.should("format ddMMyyyy to 11082006 ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, "ddMMyyyy"), "11082006");
        });

        it.should("format q to 6 ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, "q"), "3");
        });

        it.should("format w, q formatted to 32, 6 ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, "w, q"), "32, 3");
        });

        it.should("format D w, q to 223 32, 6 ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, "D w, q"), "223 32, 3");
        });
        ...
        it.should("EEEE, MMMM dd, yyyy to Friday, August 11, 2006 ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, "EEEE, MMMM dd, yyyy", true), "Friday, August 11, 2006");
        });

        it.should("EEEE, MMMM dd, yyyy G to Friday, August 11, 2006 AD ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, "EEEE, MMMM dd, yyyy G", true), "Friday, August 11, 2006 AD");
        });

        it.should("EEEE, MMMM dd, yyyy GGGG to Friday, August 11, 2006 Anno Domini ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, "EEEE, MMMM dd, yyyy GGGG", true), "Friday, August 11, 2006 Anno Domini");
        });

        it.should("M/dd/yy to 8/11/06 ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, "M/dd/yy", true), "8/11/06");
        });

        it.should("E to 6 ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, "E", true), "6");
        });

        it.should("h:m a to 5:55 AM ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, "h:m a", true), "5:55 AM");
        });

        it.should("H:m:s to 6:55:12", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, 'h:m:s', true), "5:55:12");
        });

        it.should("h:m:s.SS to 6:55:12.35 ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, 'h:m:s.SS', true), "5:55:12.35");
        });

        it.should("k:m:s.SS to 6:55:12.35 ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, 'k:m:s.SS', true), "5:55:12.35");
        });
        ...
        it.should("ddMMyyyy to 11082006 ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, "ddMMyyyy", true), "11082006");
        });

        it.should("q to 6 ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, "q", true), "3");
        });

        it.should("w, q formatted to 32, 6 ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, "w, q", true), "32, 3");
        });

        it.should("D w, q to 223 32, 6 ", function () {
            var date = new Date(2006, 7, 11, 0, 55, 12, 345);
            assert.equal(comb.date.format(date, "D w, q", true), "223 32, 3");
        });
    ...
    it.describe("when finding the diff/add", function (it) {
        ...
        it.should("find the diff/add", function () {
            var dtA = null; // First date to compare
            var dtB = null; // Second date to compare
            var interv = ''; // Interval to compare on (e.g., year, month)
            interv = "year";
    ...
    it.describe("when using convenice methods *fromNow *ago", function (it) {
        ...
        it.should("add/subtract", function () {
            assert.deepEqual(comb.yearsFromNow(2), comb.date.add(new Date(), "years", 2));
            assert.deepEqual(comb.yearsAgo(2), comb.date.add(new Date(), "years", -2));

            assert.deepEqual(comb.monthsFromNow(2), comb.date.add(new Date(), "months", 2));
            assert.deepEqual(comb.monthsAgo(2), comb.date.add(new Date(), "months", -2));

            assert.deepEqual(comb.daysFromNow(2), comb.date.add(new Date(), "days", 2));
            assert.deepEqual(comb.daysAgo(2), comb.date.add(new Date(), "days", -2));

            assert.deepEqual(comb.hoursFromNow(2), comb.date.add(new Date(), "hours", 2));
            assert.deepEqual(comb.hoursAgo(2), comb.date.add(new Date(), "hours", -2));

            assert.deepEqual(comb.minutesFromNow(2), comb.date.add(new Date(), "minutes", 2));
            assert.deepEqual(comb.minutesAgo(2), comb.date.add(new Date(), "minutes", -2));

            assert.deepEqual(comb.secondsFromNow(2), comb.date.add(new Date(), "seconds", 2));
