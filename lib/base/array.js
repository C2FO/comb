    ...
    if (isArray(arr)) {
        ...
        if (!length) {
            ...
            ret = arr.reduce(function (a, b, i) {
                if (length > 1) {
                    var ret = permute(b, rotate(copy, i).slice(1), length);
                } else {
                    ret = [
                        [b]
                    ];
...
function union() {
    ...
    var arrs = argsToArray(arguments);
    if (arrs.length > 1) {
        ret = removeDuplicates(arrs.reduce(function (a, b) {
            return a.concat(b);
        }, []));
    }
    return ret;
...
function pluck(arr, prop) {
    var prop = prop.split("."), result = arr.slice(0);
...
function arrayWrapper(arr) {
    ...
    methods.forEach(function (m) {
        ...
        Object.defineProperty(arr, m, {
            value:function () {
                var ret = func.apply(null, [this].concat(argsToArray(arguments)));
                return isArray(ret) ? arrayWrapper(ret) : ret;
            },
            writable:false,
            enumerable:false,
            configurable:false
...
var comb = exports, array, methods = ["zip", "sum", "avg", "sort", "min", "max", "difference", "removeDuplicates", "unique", "rotate",
    "permutations", "transpose", "valuesAt", "union", "intersect", "powerSet", "cartesian", "compact",
    "multiply", "flatten", "pluck"];
 ...
 * @ignoreCode
 */
comb.array = arrayWrapper;


array = merge(comb.array, {
    toArray:toArray,
    sum:sum,
    avg:avg,
    sort:sort,
