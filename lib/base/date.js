 ...
 *
 * @param {} obj the thing to test if it is a Date
 *
...
function getDayOfYear(/*Date*/dateObject) {
    // summary: gets the day of the year as represented by dateObject
    return date.difference(new Date(dateObject.getFullYear(), 0, 1, dateObject.getHours()), dateObject) + 1; // Number
}

function getWeekOfYear(/*Date*/dateObject, /*Number*/firstDayOfWeek) {
...
var date = exports.date = {
    ...
    difference : function(/*Date*/date1, /*Date?*/date2, /*String*/interval) {
        ...
        switch (interval) {
            ...
            case "weekday":
            case "weekdays":
                var days = round(date.difference(date1, date2, "day"));
     ...
     * @param {String} format the format of the date for example
                        ...
                        case 'y':
                            if (v < 100) {
                                ...
                                var year = '' + new Date().getFullYear(),
                                        ...
                                        cutoff = min(Number(year.substring(2, 4)) + 20, 99);
    ...
    format : function(date, format) {
        return format.replace(/([a-z])\1*/ig, function(match) {
            var s, pad, h,
                    c = match.charAt(0),
                    l = match.length;
            switch (c) {
                case 'G':
                    s = ((l < 4) ? eraAbbr : eraNames)[date.getFullYear() < 0 ? 0 : 1];
                    break;
                case 'y':
                    s = date.getFullYear();
                    switch (l) {
                ...
                case 'M':
                    ...
                    if (l < 3) {
                        ...
                        s = (l == 3 ? monthAbbr : monthNames)[m];
                    }
                    break;
                case 'w':
                    var firstDay = 0;
                    s = getWeekOfYear(date, firstDay); pad = true;
                    break;
                case 'd':
                    s = date.getDate();
                    pad = true;
                ...
                case 'k':
                    ...
                    break;
                case 'm':
                    s = date.getMinutes(); pad = true;
                    break;
                case 's':
                    s = date.getSeconds(); pad = true;
                    break;
                case 'S':
                    s = round(date.getMilliseconds() * pow(10, l - 3));
                    pad = true;
                    break;
                case 'v':
                case 'z':
                    s = date.getTimezoneName(date, true, options);
                    if (s) {
                        break;
                    }
                    l = 4;
                ...
                case 'Z':
                    ...
                    var tz = [
                        (offset <= 0 ? "+" : "-"),
                        string.pad(floor(abs(offset) / 60), 2),
                        string.pad(abs(offset) % 60, 2)
                    ];
                    if (l == 4) {
                        tz.splice(0, 0, "GMT");
                        tz.splice(3, 0, ":");
