...
/**
 * Wraps a stream in a promise waiting for an `"end"`, `"error"`, or if provided, a custom resolve event to be triggered.
 *
 * ```
 ...
 * });
 *
 * // or for streams that emit a "finish" event
 * var writeStream = fs.createWriteStream("my.file");
 * comb.promisfyStream(writeStream, "finish").chain(function () {
 ...
 * @param resolveEvent (optional) the stream event that resolves the promise.  If no resolve event is provided the promise will resolve with `"end"`.
 * @return {comb.Promise} a Promise is resolved if `"end"` or custom resolve event is triggered before `"error"` or rejected if `"error"` is triggered.
 * @memberOf comb
 * @static
 */
function promisfyStream(stream, resolveEvent) {
    var ret = new Promise(), called, errorHandler, endHandler;

    if (typeof resolveEvent === "undefined") {
        resolveEvent = "end";
    }

    function handle(handleFunc) {
        return function streamHandle() {
            if (!called) {
                called = true;
                spreadArgs(handleFunc, arguments, ret);
                stream.removeListener("error", endHandler);
                stream.removeListener(resolveEvent, endHandler);
                stream = ret = null;
            }
        };
    }

    errorHandler = handle(ret.errback);
    endHandler = handle(ret.callback);

    stream.on("error", errorHandler).on(resolveEvent, endHandler);
    return ret.promise();
}
