    ...
    if (l) {
        var exec = function(i) {
            if (i < l) {
                ...
                try {
                    var p = base.isUndefined(args) ? obj[name] : name == "__set__" ? obj[args[0]] = getValueFromArrayMap(args[1], pMap) : obj[name].apply(obj, args.map(function(arg) {
...
exports.executeInOrder = function() {
    var args = base.argsToArray(arguments);
    var cb = base.isFunction(args[args.length - 1]) ? args.pop() : null;
    var ret = new Promise();
    if (cb) {
        var stack = [];
