...
var Logger = (logging.Logger = define.define(null, {
    ...
    instance:{
        ...
        log:function (level, message) {
            var args = base.argsToArray(arguments).slice(1);
...
exports.logger.configure = function configure() {
    var args = base.argsToArray(arguments), configurator;
    if (!args.length) {
        configurator = new configurators.BasicConfigurator();
    } else {
        var first = args[0];
        if (base.isHash(first) || base.isString(first)) {
