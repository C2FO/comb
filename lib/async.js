...
function asyncLoop(promise, cb, scope, limit) {
    return when(promise).chain(function (results) {
        return serial((isArray(results) ? results : [results]).map(function () {
            var args = arguments;
            return function () {
                try {
                    return when(cb.apply(scope || null, args));
                } catch (e) {
                    throw e;
                }
            };
...
function asyncForEach(promise, iterator, scope) {
    return asyncArray(asyncLoop(promise, iterator, scope).chain(function (results) {
        return results.arr;
    }));
}
...
function asyncValuesAt(arrPromise) {
    ...
    return asyncArray(when(arrPromise).chain(function (result) {
        return valuesAt.apply(array, [normalizeResult(result)].concat(args));
    }));
}
    ...
    if (!p || !p.__isArrayAsync__) {
        ...
        methods.forEach(function (m) {
            ret[m] = function () {
                ...
                process.nextTick(function () {
                    asyncExports[m].apply(null, [ret].concat(args)).then(mRet);
                });
